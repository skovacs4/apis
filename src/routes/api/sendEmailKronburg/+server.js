...
import { EMAIL_KRONBURG, EMAIL_PASSWORD_KRONBURG } from "$env/static/private";
...
export async function POST({ request }) {
    ...
    if (fromDisplayName && fromDisplayName.trim() !== "") {
        sender = `"${fromDisplayName}" <${EMAIL_KRONBURG}>`;
    } else {
        // Fallback display name if fromDisplayName is not provided or empty
        sender = `"Contact" <${EMAIL_KRONBURG}>`;
    }
